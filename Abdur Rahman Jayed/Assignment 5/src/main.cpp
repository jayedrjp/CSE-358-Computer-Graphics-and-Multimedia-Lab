...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float scaleFactor = sin(timeValue) * 0.5f + 1.0f; // Sin gives a value between -1 and 1, so scale from 0.5 to 1.5
        ...
        float whiteValue = static_cast<float>(sin(timeValue) / 2.0f) + 0.5f;
 
    
        int vertexColorLocation = glGetUniformLocation(shaderProgram, "ourColor");
        glUseProgram(shaderProgram);
        glUniform4f(vertexColorLocation, 1.0f ,redValue,whiteValue, 1.0f);
